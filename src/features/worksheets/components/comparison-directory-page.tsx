  ...
  return (
    <PublicPageShell active="compare">
      ...
      <main className="hs-container pb-16 pt-6">
        <nav aria-label="Breadcrumb" className="text-sm text-[#617084]"><Link href="/"><UiText>{"Home"}</UiText></Link> <span aria-hidden="true"> / </span> <span aria-current="page">Chinese Comparisons</span></nav>
        <header className="mt-6 border-b border-[#ded7ca] pb-10 text-center">
          <p className="hs-kicker"><UiText>{"Make the decision, not a guess"}</UiText></p>
          <h1 className="hs-display mx-auto mt-3 max-w-4xl text-4xl font-bold leading-tight sm:text-5xl"><UiText>{"Chinese characters learners confuse—and how to choose"}</UiText></h1>
        ...
        <section className="mt-11 grid gap-5 rounded border border-[#233e62] bg-[#172b49] p-7 text-white sm:p-9 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          ...
          <div className="flex flex-wrap gap-3"><Link href="/generator" className="hs-primary-button min-h-12"><UiText>{"Create worksheet"}</UiText></Link><Link href="/hsk" className="hs-secondary-button min-h-12 border-white/30 bg-transparent text-white hover:bg-white/10"><UiText>{"Browse HSK lists"}</UiText></Link></div>
        </section>
      </main>
    </PublicPageShell>
  );
